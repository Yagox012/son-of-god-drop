...
function ProductCap({ progress, revealProgress }: { progress: number; revealProgress: number }) {
  ...
  useFrame((_, delta) => {
    ...
    group.current.position.y = THREE.MathUtils.damp(group.current.position.y, 0.33, 7, delta);
    group.current.scale.setScalar(THREE.MathUtils.damp(group.current.scale.x, 1.265, 7, delta));
